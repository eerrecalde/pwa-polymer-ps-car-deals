...
// 3) SW is updated to a new version if next time browser loads, sw binary comparisson doesn't match

self.addEventListener('activate', function(event){
  console.log('From SW: Activate event', event)
  // 2) To set the new SW as active
  self.clients.claim();
  event.waitUntil(
    caches.keys()
...
function cacheFirstStrategy(request) {
  return fetchRequestAndCache(request).catch(function(networkResponse) {
